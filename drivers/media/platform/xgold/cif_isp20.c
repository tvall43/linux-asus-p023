	...
	struct cif_isp20_device *dev)
{
	...
	if (!CIF_ISP20_PIX_FMT_IS_RAW_BAYER(
		dev->config.mi_config.mp.output.pix_fmt)) {
		bool capture;
		enum cif_isp20_colorspace colorspace =
			dev->config.mi_config.mp.output.colorspace;
		enum cif_isp20_pix_fmt pix_fmt =
			dev->config.mi_config.mp.output.pix_fmt;

		capture = (colorspace == CIF_FULL_COLOR_RANGE) ||
			(CIF_ISP20_PIX_FMT_IS_JPEG(pix_fmt) &&
	...
	struct cif_isp20_device *dev)
{
	...
	if ((dpcl & CIF_VI_DPCL_CHAN_MODE_MP) && 
	    (dpcl & CIF_VI_DPCL_CHAN_MODE_SP) && (dpcl & CIF_VI_DPCL_MP_MUX_MRSZ_JPEG)) {
		mi_ctrl_shd = cif_ioread32(dev->config.base_addr + CIF_MI_CTRL_SHD);
		cif_isp20_pltfrm_pr_dbg(dev->dev,
...
static int cif_isp20_isr(void *cntxt)
{
	...
	if (cif_ioread32(dev->config.base_addr + CIF_MIPI_MIS))
