...
u16 total_24 = 0, total_32 = 0, check_sum_24 = 0, check_sum_32 = 0;

extern int Read_PCB_ID(void);

static u8 ov5670_otp_data[32];

static ssize_t ov5670_read_otp(struct device *dev, struct device_attribute *attr, char *buf)
{
	pr_info("%s: get ov5670 module OTP %s !!\n", __func__, camera_module_otp);

	return sprintf(buf, "%s", camera_module_otp);
}

static DEVICE_ATTR(ov5670_read_otp, S_IRUGO, ov5670_read_otp, NULL);

static ssize_t ov5670_read_otp_uid(struct device *dev, struct device_attribute *attr, char *buf)
{
	pr_info("%s: get ov5670 module OTP UID %s !!\n", __func__, camera_module_otp_uid);

	return sprintf(buf, "%s", camera_module_otp_uid);
}

static DEVICE_ATTR(ov5670_read_otp_uid, S_IRUGO, ov5670_read_otp_uid, NULL);

static struct attribute *ov5670_attributes[] = {
	&dev_attr_ov5670_read_otp.attr,
	&dev_attr_ov5670_read_otp_uid.attr,
        NULL
};

static struct attribute_group ov5670_otp_attr = {
	.name = "ov5670_otp",
...
static const struct ov_camera_module_reg OV5670_352x288_30fps_2lanes[] = {
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x3623, 0x04},
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x3808, 0x01},
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x380e, 0x01},
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x3814, 0x07},
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x3821, 0xc6},
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x382a, 0x07},
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x450a, 0x02},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x450b, 0x00},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4600, 0x00},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4601, 0x24},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4700, 0xa4},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4800, 0x6c}, //discontinue mode
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x5045, 0x05}, // [2] enable MWB manual bias},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x5048, 0x10}, // MWB manual bias // need to be the same with 0x4003 BLC target.
};
...
static const struct ov_camera_module_reg OV5670_1296x972_30fps_2lanes[] = {
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x030b, 0x00},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x030c, 0x00},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x030d, 0x1c},//for MCLK 26MHz ASUS only test
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x030e, 0x00},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x030f, 0x06},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x0312, 0x01},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x3000, 0x00},
...
static const struct ov_camera_module_reg OV5670_1280x720_30fps_2lanes[] = {
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4700, 0xa4},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4800, 0x6c}, //OV1280x720 //discontinue mode
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4816, 0x53},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x481f, 0x40},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x4837, 0x13},
	{OV_CAMERA_MODULE_REG_TYPE_DATA, 0x5000, 0x56},
...
/*
	{
		...
		.reg_table = (void *)OV5670_352x288_30fps_2lanes,
		.reg_table_num_entries =
			sizeof(OV5670_352x288_30fps_2lanes)
			/
			sizeof(OV5670_352x288_30fps_2lanes[0]),
	...
	u32 exp_time)
{
	int ret;
	u32 vts_new, vts_old;
	u32 exp_old, exp_3rd, exp_2nd, exp_1st;

	ret = ov_camera_module_read_reg_table(cam_mod,
		OV5670_AEC_PK_LONG_EXPO_3RD_REG,&exp_3rd);
	ret |= ov_camera_module_read_reg_table(cam_mod,
		OV5670_AEC_PK_LONG_EXPO_2ND_REG,&exp_2nd);
	ret |= ov_camera_module_read_reg_table(cam_mod,
		OV5670_AEC_PK_LONG_EXPO_1ST_REG,&exp_1st);
	exp_old = (((exp_3rd & 0xf) << 16) | ((exp_2nd & 0xff) << 8) | (exp_1st & 0xff)) >> 4;

	ret = OV5670_g_VTS(cam_mod, &vts_old);

	if ((cam_mod->exp_config.exp_time + OV5670_COARSE_INTG_TIME_MAX_MARGIN)
		> cam_mod->vts_min)
		vts_new = cam_mod->exp_config.exp_time+
			OV5670_COARSE_INTG_TIME_MAX_MARGIN;
	else
		vts_new = cam_mod->vts_min;

	ov_camera_module_pr_debug(cam_mod, "exp_old = %d, exp_new = %d, vts_old =%d, vts_new = %d\n", exp_old, cam_mod->exp_config.exp_time, vts_old, vts_new);
	if((vts_new < vts_old) && (exp_old + OV5670_COARSE_INTG_TIME_MAX_MARGIN > vts_new)) {
		ov_camera_module_pr_debug(cam_mod, "Modify VTS setting\n");
		vts_new = exp_old + OV5670_COARSE_INTG_TIME_MAX_MARGIN;
	}
...
static int ov5670_write_aec(struct ov_camera_module *cam_mod)
{
	...
	if((cam_mod->exp_config.exp_time == 0) | (cam_mod->exp_config.gain == 0))
		return ret;
	...
	if ((cam_mod->state == OV_CAMERA_MODULE_SW_STANDBY) ||
		...
		//ASUS_BSP+++
		u32 total_gain = cam_mod->exp_config.gain;
		u32 a_gain = cam_mod->exp_config.gain;
		u32 d_gain = 64;
		u32 exp_time = cam_mod->exp_config.exp_time << 4;

		if(total_gain > 0x1000) //32X
			total_gain = 0x1000;
		if(total_gain > 1984) { //15.5X
			a_gain = 1984;
			d_gain = total_gain*64/a_gain;
		} else {
			a_gain = total_gain;
			d_gain = 64;
		}
		...
		ov_camera_module_pr_debug(cam_mod,
			"total_gain = %d, a_gain = %d, d_gain = %d\n", total_gain, a_gain, d_gain);
		...
		ret |= ov_camera_module_write_reg(cam_mod,
			...
			OV5670_AEC_GROUP_UPDATE_START_DATA); //3208 00, grp 0 hold start
		...
		for(i=0; i<3; i++)
		{
			ret |= ov_camera_module_write_reg(cam_mod, 0x5032+i*2, ((d_gain << 4) & 0x0F00) >> 8);
			ret |= ov_camera_module_write_reg(cam_mod, 0x5033+i*2, ((d_gain << 4) & 0x00FF));
		}
/*
		//set 0x5045[2]=1 to enable manual bias
		ret |= ov_camera_module_read_reg(cam_mod, 1, 0x5045, &read_reg);
		ret |= ov_camera_module_write_reg(cam_mod, 0x5045, (read_reg|0x4));

		//set 0x5048 to the black level target set in 0x4003
		ret |= ov_camera_module_read_reg(cam_mod, 1, 0x4003, &read_reg);
		ret |= ov_camera_module_write_reg(cam_mod, 0x5048, read_reg);
*/
		ret |= ov_camera_module_write_reg(cam_mod,
			OV5670_AEC_GROUP_UPDATE_ADDRESS,
	...
	struct ov_camera_module_timings *timings)
{
	...
	//timings->line_length_pck = (timings->line_length_pck)*2; //DIT
...
int __ov5670_read_otp(struct v4l2_subdev *sd, struct ov5670_af_data *buf)
{
	...
	ov_camera_module_read_reg(cam_mod, 1, 0x5002, &temp1);
	ov_camera_module_write_reg(cam_mod, 0x5002, ((0x00&0x02) | (temp1&(~0x02))));
	...
	for (package = 2; package >= 0; package--){
		...
		ov_camera_module_write_reg(cam_mod, 0x3d88, (start_addr >> 8)&0xff);
		ov_camera_module_write_reg(cam_mod, 0x3d88, start_addr &0xff);

		// partial mode OTP write end address
		ov_camera_module_write_reg(cam_mod, 0x3d8A, (end_addr >> 8) & 0xff);
		ov_camera_module_write_reg(cam_mod, 0x3d8B, end_addr & 0xff);
	...
	//set 0x5002[1] to “1”
	ov_camera_module_read_reg(cam_mod, 1, 0x5002, &temp1);
	ov_camera_module_write_reg(cam_mod, 0x5002, (0x02 & 0x02) | (temp1 & (~0x02)));
	return -EIO;
...
out:
	...
	ov_camera_module_read_reg(cam_mod, 1, 0x5002, &temp1);
	ov_camera_module_write_reg(cam_mod, 0x5002, (0x02 & 0x02) | (temp1 & (~0x02)));
	printk("%s Total_24 = %x, check_sum_24 = %x Total_32 = %x, check_sum_32 = %x\n", __func__, total_24, check_sum_24, total_32, check_sum_32);

	//Return OTP value
	if (total_32 != check_sum_32) {
		//Return OTP value
		snprintf(camera_module_otp, sizeof(camera_module_otp), "0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X\n"
			...
			, read_value[20], read_value[21], read_value[22], read_value[23]);
	} else {
		//Return OTP value
...
static int ov5670_otp_read(struct v4l2_subdev *sd)
{
	...
	ov_camera_module_write_reg(cam_mod, 0x0100, 0x01);
	...
	if (ret) {
		dev_info(&client->dev, "failed to read OTP data...\n");
	...
	const struct i2c_device_id *id)
{
	...
	pcb_id = Read_PCB_ID();
	if(((pcb_id>>11) & 0x3) != 2) {
		dev_info(&client->dev, "2M SKU, bypass 5M probe\n");
		return -1;
	}
	#endif
	...
	//ASUS_BSP+++, OTP
	ov5670_otp_read(&ov5670.sd);
	sysfs_create_group(&client->dev.kobj, &ov5670_otp_attr);
	//ASUS_BSP---, OTP
	...
	struct i2c_client *client)
{
	...
	sysfs_remove_group(&client->dev.kobj, &ov5670_otp_attr); //ASUS_BSP+++
