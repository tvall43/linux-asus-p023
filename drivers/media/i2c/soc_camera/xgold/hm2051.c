...

{
	...
	ret = hm2051_read_reg(client, HM2051_8BIT, HM2051_IMG_OUT_CONF, &img_out_conf);
   
        if (ret) {
                dev_err(&client->dev, "HM2051_IMG_OUT_CONF err\n");
        }

        if (coarse_itg < (dev->sensor_blanking - 2 + hm2051_res[dev->fmt_idx].lines_per_frame)) {
                img_out_conf |= HM2051_FIXED_FRAME_RATE_MASK;
                hm2051_write_reg(client, HM2051_8BIT, HM2051_IMG_OUT_CONF, img_out_conf);
        }
        else {
                img_out_conf &= (~HM2051_FIXED_FRAME_RATE_MASK);
                hm2051_write_reg(client, HM2051_8BIT, HM2051_IMG_OUT_CONF, img_out_conf);
			     ...
			     struct v4l2_mbus_framefmt *fmt)
{
	...
	if (ret)
        ...
        /*
        ret = hm2051_read_reg(client, HM2051_8BIT, HM2051_REG_BLANKING_ROW_H, &high);
        if (ret) {
                dev_err(&client->dev, "hm2051 HM2051_REG_BLANKING_ROW_H err\n");
        }
        ret = hm2051_read_reg(client, HM2051_8BIT, HM2051_REG_BLANKING_ROW_L, &low);
        if (ret) {
                dev_err(&client->dev, "hm2051 HM2051_REG_BLANKING_ROW_L err\n");
        }
...
#endif
	ATD_hm2051_status = 1;
	printk("hm2051_detect: Sensor ID: 0x%x\n", id);
...
static int hm2051_dbgfs_init(void)
{
	...
	debugfs_create_u32("camera_status", 0644, debugfs_dir, &ATD_hm2051_status);
			...
			const struct i2c_device_id *id)
{
	...
	pcb_id = Read_PCB_ID();
	if(((pcb_id>>11) & 0x3) != 3) {
		dev_info(&client->dev, "5M SKU, bypass 2M probe\n");
		return -1;
	}
	#endif
