...
{
#if defined(SDHCI_ENABLED_CARD_DETECTION)
...
#if !defined(SDHCI_ENABLED_CARD_DETECTION)
    sdhci_mask_irqs(host, SDHCI_INT_CARD_REMOVE | SDHCI_INT_CARD_INSERT);
#endif
}
...
static int sdhci_do_get_cd(struct sdhci_host *host)
{
    ...
    /* If platform card detection has been specified. */
    if (host->ops->platform_get_cd)
        gpio_cd = (host->ops->platform_get_cd)(host);
...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	if (host->runtime_suspended) {
		...
		pr_warn_ratelimited("%s: got irq while runtime suspended\n",
                            mmc_hostname(host->mmc));
		ier = sdhci_readl(host, SDHCI_INT_ENABLE);
		ser = sdhci_readl(host, SDHCI_SIGNAL_ENABLE);
		ist = sdhci_readl(host, SDHCI_INT_STATUS);
		pr_warn_ratelimited("%s: ier: 0x%08x, ser: 0x%08x ist: 0x%08x\n",
				mmc_hostname(host->mmc), ier, ser, ist);
#if !defined(SDHCI_ENABLED_CARD_DETECTION)
        sdhci_mask_irqs(host, SDHCI_INT_CARD_REMOVE | SDHCI_INT_CARD_INSERT);
#endif
		return IRQ_HANDLED;
...
int sdhci_runtime_suspend_host(struct sdhci_host *host)
{
	...
	spin_unlock_irqrestore(&host->lock, flags);
	if (host->ops->platform_runtime_suspend)
		host->ops->platform_runtime_suspend(host);
...
int sdhci_runtime_resume_host(struct sdhci_host *host)
{
	...
	if (host->ops->platform_runtime_resume)
		host->ops->platform_runtime_resume(host);
