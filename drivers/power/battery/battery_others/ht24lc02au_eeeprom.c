...
static int ht24lc02au_read(struct ht24lc02au_eeprom *ht24, u8 reg)
{
    ...
    do
    {
        wake_lock(&ht24->wakelock);
        ret = i2c_smbus_read_byte_data(ht24->client, reg);
        wake_unlock(&ht24->wakelock);
                ...
                u8 *val, int ifDebug)
{
    ...
    wake_lock(&ht24lc02au_chg->wakelock);
    ret = i2c_smbus_read_byte_data(ht24lc02au_chg->client, reg);
    wake_unlock(&ht24lc02au_chg->wakelock);
            ...
            const struct i2c_device_id *id)
{
    ...
    ht24->dev = dev;
    ht24lc02au = ht24;
    wake_lock_init(&ht24->wakelock, WAKE_LOCK_SUSPEND, "ht24lc02au");
