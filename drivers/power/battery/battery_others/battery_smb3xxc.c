                    ...
                    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_ONLINE:
        if (COVER_ATTACHED_UPI() && IS_CB81()) {
            if (COVER_OTG_PIN_CONTROL_ENABLE())
                val->intval = 1;
            else if (entry_mode == 4 && DCIN_VBUS())
                val->intval = 1;
            else
                val->intval = 0;
        }
...
#if defined(CONFIG_Z380KL) || defined(CONFIG_Z380C)
int smb3xxc_jeita_control(int bat_tempr, int bat_volt)
{
...
#if defined(CONFIG_Z380KL) || defined(CONFIG_Z380C)
int smb3xxc_jeita_control_for_sw_gauge(int usb_state)
{
    int ret;
    ...
    /* exec jeita function */
    ret = smb3xxc_jeita_control(batt_tempr, batt_volt);

    return ret;
}
#else
int smb3xxc_jeita_control_for_sw_gauge(int usb_state) { return 0; }
#endif
...
int smb345c_get_charging_status(void)
{
    ...
    irqstat_c = smb345c_read(smb345c_dev, IRQSTAT_C);
    if (irqstat_c < 0)
        return POWER_SUPPLY_STATUS_UNKNOWN;
#if 0
...
static int smb345c_parse_dt(struct smb345c_charger *chip)
{
    ...
    chip->cover_i2c_enable_gpio =
        of_get_named_gpio(node, "intel,cover-i2c-enable-gpio", 0);
    pr_info("intel,cover-i2c-enable-gpio = %d.\n", chip->cover_i2c_enable_gpio);
            ...
            const struct i2c_device_id *id)
{
    ...
    /* qcom */
    smb->dev = &client->dev;
    wake_lock_init(&smb->wakelock, WAKE_LOCK_SUSPEND, "smb358");
