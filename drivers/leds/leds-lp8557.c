...
static int lp8557_post_init_device(struct i2c_client *client)
{
	...
	if (ret < 0)
		return -1;
#if defined(ASUS_PROJECT_Z380C_DISPLAY)
	...
	ret = send_i2c_command(client, 0x15, 0xc3);
	ret = send_i2c_command(client, 0x7F, 0x21);
	ret = send_i2c_command(client, 0x7A, 0x88);
	ret = send_i2c_command(client, 0x16, 0x60);
	ret = send_i2c_command(client, 0x14, 0x9f);
#else
	ret = send_i2c_command(client, 0x10, 0x84);
	ret = send_i2c_command(client, 0x11, 0x05);
	ret = send_i2c_command(client, 0x12, 0x2c);
	ret = send_i2c_command(client, 0x13, 0x03);
	ret = send_i2c_command(client, 0x14, 0x1f);
	ret = send_i2c_command(client, 0x15, 0xc3);
#endif
