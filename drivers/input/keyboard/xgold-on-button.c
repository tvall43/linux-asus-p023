...
static void __on_button_press_isr(struct xgold_on_button_pdata *data)
{
	...
	if (data->input_dev) {
		...
		dev_info(dev, "Power key pressed - event reported for %d, %d\n",
			KEY_POWER, 1);
		input_sync(data->input_dev);
...
static void __on_button_release_isr(struct xgold_on_button_pdata *data)
{
	...
	if (data->input_dev) {
		...
		dev_info(dev, "Power key released - event reported for %d, %d\n",
			KEY_POWER, 0);
		input_sync(data->input_dev);
                wake_unlock(&keypad_wake_lock);
	}
...
static int32_t xgold_on_button_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(np, PROP_UTILITY_BUTTON, NULL)) {
        ...
        wake_lock_init(&keypad_wake_lock, WAKE_LOCK_SUSPEND, "keypad_wake_lock");
