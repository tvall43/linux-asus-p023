...
#include <linux/i2c.h>
#include <linux/i2c/atmel_mxt_ts_1666t2.h>
...
#define mxt_stylus 1
#define mxt_zstylus_only_mode 1
...
#ifdef mxt_zstylus_only_mode
#define MXT_PROC_ZSTYLUS_ONLY_MODE_FILE		"mxt_zstylus_only_mode"
static struct proc_dir_entry *mxt_proc_zstylus_only_mode_file = NULL;
#endif
...
static void mxt1666t2_screen_chenged_listaner(const int state);
#endif
...
extern int panel_index;
static int have_stylus = 0;
...
struct mxt_data {
	...
	u8 gesture_type_6;
	u8 z_stylus;
	u8 z_stylus_only_mode;
	int z_stylus_only_mode_second;
	...
	struct delayed_work report_delay_work; //if stylus == 1, finger can report after 1s.
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		if(current_batlvl == 0) current_batlvl = 3;
...
retry_read:
	...
	if (ret == 1) {
		...
		if(retry_count < 3){
			retry_count++;
			dev_err(&data->client->dev, "%s: i2c transfer failed (%d) ,retry:%d\n",
				__func__, ret, retry_count);
			mdelay(50 * retry_count);
			goto retry_read;
		} else {
			dev_err(&data->client->dev, "%s: i2c transfer failed (%d)\n",
				__func__, ret);
			return -EIO;
		}
	}
...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		...
		if(retry_count < 3){
			retry_count++;
			dev_err(&client->dev, "%s: i2c transfer failed (%d) ,retry:%d\n",
				__func__, ret, retry_count);
			mdelay(50 * retry_count);
			goto retry_read;
		} else {
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (status & MXT_T100_DETECT) {
		...
		switch (type) {
			...
			case MXT_T100_TYPE_GLOVE:
			    ...
			    if(have_stylus == 1 && touch_chip->z_stylus_only_mode == 1){
                    input_mt_slot(input_dev, id);
                    input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, 0);
                    return;
                }
			...
			case MXT_T100_TYPE_ACTIVE_STYLUS:
				...
				if(touch_chip->z_stylus_only_mode == 1){
                    have_stylus = 1;
					power_supply_changed(&asus_pen_power_supplies[0]);
                }
                active = true;
                cancel_delayed_work(&data->report_delay_work);
	...
	if(active){	
		if (type == MXT_T100_TYPE_ACTIVE_STYLUS){
			...
			cancel_delayed_work(&data->stylus_batlvl_work);
		...
		if(have_stylus == 1 && touch_chip->z_stylus_only_mode == 1){
			//have_stylus = 0;
			queue_delayed_work(maxtouch_wq, &data->report_delay_work, touch_chip->z_stylus_only_mode_second*HZ);
		}
...
static irqreturn_t mxt_process_messages_t44(struct mxt_data *data)
{
	...
	while(retry < 3){
		...
		if (ret) {
			if(retry < 3){
				dev_err(dev, "Failed to read T44 and T5 (%d), retry:%d\n", ret,retry);
				retry++;
			} else {
				return IRQ_NONE;
			}
			       ...
			       unsigned int data_pos, int config_match)
{
	...
	//for second source TP
	if((config_MSB == 1) && (data->maj_version != 1) && (panel_index == 1)){
		dev_err(dev, "File config is second source, and Panel Index is \n");
		return 1;
	}
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *cfg, int config_check)
{
	...
	if (info_crc == data->info_crc) {
		if (config_crc == 0 || data->config_crc == 0) {
			...
			dev_err(dev, "Config CRC 0x%06X: does not match file 0x%06X\n",
...
struct device *dev = &data->client->dev;
	...
	if (IS_ERR(data->reg_vdd)) {
	    ...
	    if (error)
	    {
	        dev_err(dev, "Regulator set_vtg failed vdd error=%d\n", error);
	        goto fail;
	    }
	    ...
	    if (error)
	    {
	        dev_err(dev, "Regulator vdd enable failed error=%d\n", error);
	        goto fail;
	    }
	}
	...
	if (IS_ERR(data->reg_avdd)) {
	    ...
	    if (error)
	    {
	        dev_err(dev, "Regulator avdd enable failed error=%d\n", error);
	        goto fail_release;
	    }
	}
...
static int mxt_set_t7_power_cfg(struct mxt_data *data, u8 sleep)
{
	...
	*/
	...
	//printk("[Atmel] Set T7 ACTV:%d IDLE:%d\n",new_config->active, new_config->idle);
				 ...
				 const struct firmware *cfg, int config_check)
{
	...
	while(retry < 3){
		...
		if (error) {
			...
			retry++;
			mdelay(500);
			dev_err(dev, "mxt_init_t7_power_cfg retry %d!!!\n",retry);
			//goto err_free_object_table;
		}else{
			break;
		}
...
static int mxt_load_cfg(struct device *dev)
{
	...
	if(panel_index == 1){
		error = request_firmware(&cfg, MXT_CONFIG_NAME_1666T2_SECOND, dev);
	} else {
...
static int mxt_self_update_firmware(struct mxt_data *data){
	...
	//printk("[Atmel] %s ++ %d,%d\n",__func__,(data->info->version),(data->info->build) );
	if((NEWEST_FW_VERSION != chip_fw_version) || (NEWEST_FW_BUILD != chip_fw_build)){
		...
		if(panel_index == 1){
			touch_chip->cfg_name = MXT_CONFIG_NAME_1666T2_SECOND;
		} else {
		...
		if (error) {
			dev_err(dev, "The self update firmware update failed(%d)\n", error);
			...
			dev_info(dev, "The self update firmware update succeeded\n");
			if (error)
				return error;
		}
	}
...
static int mxt_self_update_cfg(struct mxt_data *data, int config_check){
	...
	wake_lock(&mxt_wake_lock);
	if(panel_index == 1){
		ret = request_firmware(&cfg, MXT_CONFIG_NAME_1666T2_SECOND, dev);
	} else {
...
static ssize_t mxt_disable_touch_store(struct device *dev,
       struct device_attribute *attr, const char *buf, size_t count)
{
       touch_chip->z_stylus_only_mode_second = buf[0] - '0';
       printk("[Ateml]: z_stylus_only_mode_second %d\n",touch_chip->z_stylus_only_mode_second);
       return count;
}
...
{
	if(build_version != 1){
		printk("[Atmel] %s ++ skip,Not in ENG mode\n",__func__);
		return 0;
	}
...
{	
	...
	printk("[Atmel] %s ,build_version:%d++ \n",__func__,build_version);
	...
	if(touch_chip->update_result == 0){
		...
		err = mxt_self_update_cfg(touch_chip, 0);
			printk("[Atmel] mxt_self_update_cfg fail,err:%d!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n",err);
		while(err != 0 && retry < 5)
		{
			mxt_update_crc(touch_chip, MXT_COMMAND_BACKUPNV, MXT_BACKUP_VALUE);
			mdelay(100);
...
static DEVICE_ATTR(enable_gpio, S_IWUSR | S_IRUSR, atmel_enable_gpio, mxt_gpio_enable_disable);
...
static DEVICE_ATTR(disable_touch, S_IWUSR | S_IRUSR, NULL, mxt_disable_touch_store);
...
static void mxt_create_proc_double_tap_file(void)
{	
...
static void mxt_remove_proc_double_tap_file(void)
{
...
static void mxt_create_proc_gesture_file(void)
{
...
static void mxt_remove_proc_gesture_file(void)
{
	extern struct proc_dir_entry proc_root;
...
static void mxt_create_proc_stylus_file(void)
{	
...
static void mxt_remove_proc_stylus_file(void)
{
	extern struct proc_dir_entry proc_root;
	printk("%s:[%d]: proc double_tap file removed.", __func__, __LINE__);
...
#ifdef mxt_zstylus_only_mode
static ssize_t mxt_proc_zstylus_only_mode_read(struct seq_file *buf, void *v)
{
       printk("%s: z_stylus_only_mode = %d, second = %d!!!!!!\n", __func__,touch_chip->z_stylus_only_mode,touch_chip->z_stylus_only_mode_second);
       return 0;
}

static ssize_t mxt_proc_zstylus_only_mode_write(struct file *filp, const char __user *buf, unsigned long len, void *data)
{
       if (buf[0]=='0'){
               touch_chip->z_stylus_only_mode = 0;
               touch_chip->z_stylus_only_mode_second = 0;
			   have_stylus = 0;
			   power_supply_changed(&asus_pen_power_supplies[0]);			   
       }else{
               touch_chip->z_stylus_only_mode = 1;
               touch_chip->z_stylus_only_mode_second = buf[2] - '0';
       }
       printk("%s: z_stylus_only_mode = %d, second = %d!!!!!!\n", __func__,touch_chip->z_stylus_only_mode,touch_chip->z_stylus_only_mode_second);
       return len;
}

static int mxt_proc_zstylus_only_mode_open(struct inode *inode, struct  file *file)
{
       return single_open(file, mxt_proc_zstylus_only_mode_read, NULL);
}

static const struct file_operations mxt_zstylus_only_mode_fops =
{
       .owner = THIS_MODULE,
       .open = mxt_proc_zstylus_only_mode_open,
       .read = seq_read,
       .write = mxt_proc_zstylus_only_mode_write,
};

static void mxt_create_proc_zstylus_only_mode_file(void)
{
       mxt_proc_zstylus_only_mode_file = proc_create(MXT_PROC_ZSTYLUS_ONLY_MODE_FILE, 0666, NULL, &mxt_zstylus_only_mode_fops);
       if(mxt_proc_zstylus_only_mode_file)
       {
               //printk("%s:: proc double_tap file create sucessed\n", __func__);
       }
       else
...
static void mxt_remove_proc_zstylus_only_mode_file(void)
{
       extern struct proc_dir_entry proc_root;
       printk("%s:[%d]: proc double_tap file removed.", __func__, __LINE__);
       remove_proc_entry(MXT_PROC_ZSTYLUS_ONLY_MODE_FILE, &proc_root);
       return;
}
#endif
...
static void mxt_create_proc_logtool_file(void)
{
...
static void mxt_remove_proc_logtool_file(void)
{
	extern struct proc_dir_entry proc_root;
...
static int mxt_touch_sysfs_init(void)
{
	...
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file update_touch_firmware_config failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_enable_gpio.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_enable_gpio failed\n");
		return ret;
	}
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_disable_gpio.attr);
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file dev_attr_disable_gpio failed\n");
		return ret;
	}
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[Atmel] %s. ver:%s. panel_ID:%d.\n",__func__,DRIVER_VERSION,panel_index);
	...
	data->z_stylus = 0;
	data->z_stylus_only_mode = 0;
    data->z_stylus_only_mode_second = 0;
	data->debug_log_enabled = 0;
	data->btn_stylus = 0;
	data->btn_stylus2 = 0;
	...
	#endif
    #ifdef mxt_zstylus_only_mode
    mxt_create_proc_zstylus_only_mode_file();
	#endif
	...
	// reg callback
#ifdef CONFIG_PM_SCREEN_STATE_NOTIFIER
    callback_struct = register_screen_state_notifier(&mxt1666t2_screen_chenged_listaner);
#endif
...
{
	printk("[Atmel] %s. %d.%d.%d. %d\n",__func__,touch_chip->suspended,touch_chip->in_bootloader,touch_chip->updating_config,hall_trigger_suspend);
	
	have_stylus = 0;
	
	if (touch_chip->update_result == 1 || touch_chip->in_bootloader == 1){
		printk("[Atmel]firmware update fail %d\n",touch_chip->update_result);
		return 0;
	}	
...
//static SIMPLE_DEV_PM_OPS(mxt_pm_ops, mxt_suspend_disable, mxt_resume_disable);
...
static int __init mxt_init(void)
{
	...
	printk("[Atmel] %s ++ I2C_3 ++, HW_ID:%d \n",__func__,HW_ID);
	if(HW_ID == HW_ID_ER){
		return 0;
	}
	// add by josh for skip COS/POS ++
    if(entry_mode != 1) {
        printk("[Atmel] Not in MOS, skip probe. No in entry_mode:%d\n",entry_mode);
        return 0;
    }
    //add by josh for skip COS/POS --
