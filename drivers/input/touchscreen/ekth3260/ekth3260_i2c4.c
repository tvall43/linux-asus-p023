...
//static int gesture_table[ELAN_GESTURE_COUNT]={0};

//static int gesture_enable=0;	// 0:sleep mode, 1:gesture mode
#endif
...
static int	PageNum		  = sizeof(file_fw_data)/132; /*for ektf2xxx/3xxx serial, the page number is 249/351*/
//int PageNum = 249/351; // add by leo for testtest
...
static int read_fw(char *fw_path)
{
	int pos=0;
...
static struct miscdevice ekth3260_misc_dev =
{
...
#ifdef IAP_PORTION
static int EnterISPMode(struct i2c_client *client)
{
	int len = 0;
...
#endif
#ifdef BURN_BY_8BYTE // 132byte mode		
		if(!ekth3260_update_outside)
			szBuff = file_fw_data + curIndex;
		else
			szBuff = firmware_outside+ curIndex;
...
static int write_ice_status=0;
static int shift_out_16(struct i2c_client *client){
	int res;
	uint8_t buff[] = {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbf,0xff};
	res = i2c_master_send(client, buff,  sizeof(buff));
	return res;
}
static int tms_reset(struct i2c_client *client){
	int res;
...
static int mode_gen(struct i2c_client *client){
	int res;
	int retry = 5;
...
static int word_scan_out(struct i2c_client *client){
	int res;
...
static int long_word_scan_out(struct i2c_client *client){
	int res;
...
static int bit_manipulation(int TDI, int TMS, int TCK, int TDO,int TDI_1, int TMS_1, int TCK_1, int TDO_1){
	int res; 
	res= ((TDI<<3 |TMS<<2 |TCK |TDO)<<4) |(TDI_1<<3 |TMS_1<<2 |TCK_1 |TDO_1);
	return res;
}

static int ins_write(struct i2c_client *client, uint8_t buf){
	int res=0;
	int length=13;
...
static int word_scan_in(struct i2c_client *client, uint16_t buf){
	int res=0;
...
static int long_word_scan_in(struct i2c_client *client, int buf_1, int buf_2){
...
static uint16_t trimtable[8]={0};

static int Read_SFR(struct i2c_client *client, int open){
...
static int Write_SFR_2k(struct i2c_client *client, int open){
...
static int Write_SFR(struct i2c_client *client){
...
static int Enter_Mode(struct i2c_client *client){
	mode_gen(client);
	tms_reset(client);
...
static int Open_High_Voltage(struct i2c_client *client, int open){
#ifdef EKTF3K_FLASH
...
static int Mass_Erase(struct i2c_client *client){
	char mass_buff[4]={0};
...
static int Reset_ICE(struct i2c_client *client){
	//struct elan_ktf_ts_data *ts = i2c_get_clientdata(client);
	int res;
...
/*
static int normal_write_func(struct i2c_client *client, int j, uint8_t *szBuff){
	//char buff_check=0;
...
*/
static int fastmode_write_func(struct i2c_client *client, int j, uint8_t *szBuff){
...
static int ektSize;
//static int lastpage_byte;
//static int lastpage_flag=0;
static int Write_Page(struct i2c_client *client, int j, uint8_t *szBuff){
	int len, finish=0;
...
static int fastmode_read_func(struct i2c_client *client, int j, uint8_t *szBuff){
...
/*
static int normal_read_func(struct i2c_client *client, int j,  uint8_t *szBuff){
	char read_buff[2];
	int m, len, read_byte;
	uint16_t szbuff=0, szbuff_1=0;
...
static int Read_Page(struct i2c_client *client, int j,  uint8_t *szBuff){
	int res=0;
	ins_write(client,0x6f);
	//long_word_scan_in(client,0x007e,0x0023);
	long_word_scan_in(client,0x007f,0x9000);
...
static int TWO_WIRE_ICE(struct i2c_client *client){
	int i;
...
static int elan_TWO_WIRE_ICE( struct i2c_client *client) // for driver internal 2-wire ice
{
	work_lock=1;
	disable_irq(private_ts->client->irq);
	//wake_lock(&private_ts->wakelock);
...
#endif
/*
static void touch_callback(unsigned cable_status){ 
...
static void force_release_pos(struct i2c_client *client)
{
	struct elan_ktf_ts_data *ts = i2c_get_clientdata(client);
	int i;
...
static int eKTH3260_parse_dt(struct device *dev, struct elan_ktf_ts_data *ts)
{
	...
	printk("////////////////////		ts->rst_gpio_gpio =%d, ts->intr_gpio =%d,  ts->power_gpio =%d\n", ts->rst_gpio, ts->intr_gpio, ts->power_gpio); // add by leo for testtest
	*/
	printk("////////////////////		ts->rst_gpio_gpio =%d, ts->intr_gpio =%d\n", ts->rst_gpio, ts->intr_gpio); // add by leo for testtest
    return 0;
}
/*
static int eKTH3260_fw_version_check(void)
{
	if((FW_ID!=0x3037)||(FW_VERSION!=0x550D))
...
static ssize_t eKTH3260_proc_gesture_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
	...
	printk("%s: GESTURE_DOUBLE_CLICK = %c, GESTURE_ENABLE = %c, GESTURE_W = %c, GESTURE_S = %c, GESTURE_E = %c, GESTURE_C = %c, GESTURE_Z = %c, GESTURE_V = %c \n", 
...
static ssize_t eKTH3260_proc_tp_debug_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
	...
	//else
	//{
	//	err = i2c_master_recv(ts->client, buf_recv, 8);
	//	printk("[elan] %s: hello packet %2x:%2X:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, 
...
static void eKTH3260_create_proc_tp_debug_file(void)
{
	...
	if(eKTH3260_proc_tp_debug_file)
		printk("%s:[%d]: proc TP debug file create sucessed\n", __func__, __LINE__);
	else
		printk("%s:[%d]: proc TP debug file create failed\n", __func__, __LINE__);
	return;
}
...
{
	extern struct proc_dir_entry proc_root;
	printk("%s:[%d]: proc debug file removed.\n", __func__, __LINE__);
...
const struct i2c_device_id *id)
{
	...
	printk("////////////////////		elan_ktf_ts_probe START I2C4 v12 \n"); // add by leo for testtest
        client->addr = 0x10;
	...
	// add by leo for ER with I2C3 ++
    	ts->HW_ID = Read_HW_ID();
    	printk("////////////////////		HW ID = %d\n", ts->HW_ID);
    	if(ts->HW_ID!=0)
	{
		printk("////////////////////		Skip probe ELAN eKTH3264 Touch driver on I2C4 !!!\n");
		return -ENODEV;
    	}
    	// add by leo for ER with I2C3 --

	printk("////////////////////		elan_ktf_ts_probe START - 1\n"); // add by leo for testtest
	...
	*/
	...
	// add by leo ++
	/*
	printk("////////////////////		elan_ktf_ts_probe START - 2\n"); // add by leo for testtest
	do {
	...
	*/
	...
	printk("////////////////////		elan_ktf_ts_probe START - 3\n"); // add by leo for testtest
	elan_ktf_ts_hw_reset();
	msleep(100);

	printk("////////////////////		elan_ktf_ts_probe START - 4\n"); // add by leo for testtest
	fw_err = elan_ktf_ts_setup(client);
	if (fw_err < 0) {
		printk(KERN_INFO "No Elan chip inside\n");
		//fw_err = -ENODEV;  
	}
	printk("////////////////////		elan_ktf_ts_probe START - 5\n"); // add by leo for testtest	
...
*/         	
	...
	if(FW_ID != 0xabcd)
	{
		printk("////////////////////		Wrong FW ID !!!!! BURN IT AGAIN\n");	
		elan_TWO_WIRE_ICE(ts->client);
		//Update_FW_in_Driver(0);
	}
	else
		printk("////////////////////		FW ID Check pass !\n");
	*/
	/*
...
// add by leo for early-suspend --
	...
	//disable_irq(ts->client->irq); // add by leo for I2C3 ++
	return 0;
...
static int elan_ktf_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(power_lock==0) /* The power_lock can be removed when firmware upgrade procedure will not be enter into suspend mode.  */
	{
		...
		if(ts->gesture & GESTURE_ENABLE) {
			i2c_master_send(client, gesture_wake_up_cmd, sizeof(gesture_wake_up_cmd));
			printk("enter gesture mode: cmd %x %x %x %x\n", gesture_wake_up_cmd[0], gesture_wake_up_cmd[1], gesture_wake_up_cmd[2], gesture_wake_up_cmd[3]);
		} else {
...
static const struct i2c_device_id elan_ktf_ts_id_i2c4[] = {
	{ ELAN_KTF_NAME_I2C4, 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, elan_ktf_ts_id_i2c4);

static const struct of_device_id elan_of_match_i2c4[] = {
	{ .compatible = "elan,ekth3260_i2c4" },
	{ /* sentinel */ }
};

static struct i2c_driver ektf_ts_driver_i2c4 = {
	.probe		= elan_ktf_ts_probe,
	.remove		= elan_ktf_ts_remove,
	//.suspend	= elan_ktf_ts_suspend,
	//.resume		= elan_ktf_ts_resume,
	.id_table	= elan_ktf_ts_id_i2c4,
	.driver		= {
		.name = ELAN_KTF_NAME_I2C4,
		.owner = THIS_MODULE,
		.of_match_table = elan_of_match_i2c4,
	},
};

#if 0
module_i2c_driver(ektf_ts_driver_i2c4);
...
static int elan_ktf_ts_init(void)
{
	...
	printk("////////////////////		elan_ktf_ts_init_i24 START !!!\n");

	// add by leo for ER with I2C3 --
    	ekth3260_hw_id = Read_HW_ID();
	//ekth3260_hw_id = 2; // add by leo for testtest
    	//printk("////////////////////		ekth3260_hw_id = %d\n", ekth3260_hw_id);
    	if(ekth3260_hw_id==0)
	{
		printk("////////////////////		probe SR ELAN eKTH3264 Touch driver !!!\n");
		return i2c_add_driver(&ektf_ts_driver_i2c4);
    	}
	else
	{
		//printk("////////////////////		probe ELAN eKTH3264 Touch driver on I2C3 !!!\n");
		//return i2c_add_driver(&ektf_ts_driver);
		printk("////////////////////		Skip SR ELAN eKTH3264 Touch driver !!!\n");
		return 0;
	}
    	// add by leo for ER with I2C3 --
}
...
{
	i2c_del_driver(&ektf_ts_driver_i2c4);
	return;
}
