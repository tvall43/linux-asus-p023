...
#define HELLOW_IN_GESTURE_WORKAROUND 0
#define GESTURE_WAKEUP_BY_RESUME_FUNCTION 1
...
//extern int xgold_i2c_function_check(void);
// add by leo for cable status notifier ++
...
{
#include "fw_data_0x552E_CPT.i"
};

// add by leo for AUO panel ++
static uint8_t file_fw_data_AUO[] =
{
#include "fw_data_0x5508_AUO.i"
};
// add by leo for AUO panel --
...
static ssize_t elan_lcm_id_show(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(lcm_id2, S_IRUGO, elan_lcm_id_show, NULL);
//add by leo --
...
static uint8_t buf[PACKET_SIZE] = { 0 }; // add by leo for testtest
...
static void elan_ktf_ts_work_func(struct work_struct *work)
{
	...
	*/
	...
	#if 0
	if((ts->system_suspend)&&(debug>20))
	{
		printk("[elan] %s: Skip work function when system suspend.\n", __func__);
		enable_irq(ts->client->irq);
		wake_unlock(&private_ts->wakelock);
		return;
	}
...
{
	int i2c_check_result = 0;

#if 1
	struct elan_ktf_ts_data *ts = private_ts;

	//i2c_check_result = xgold_i2c_function_check();
	//if(i2c_check_result)
	//{
	//	printk("[elan] %s: ////////////////////		i2c_check_result: %d\n", __func__, i2c_check_result);
	//	return IRQ_HANDLED;
	//}

#if (!GESTURE_WAKEUP_BY_RESUME_FUNCTION)
	if(ts->system_suspend)
		queue_delayed_work(ts->elan_irq_wq, &ts->irq_delay_work, 0.1 * HZ);
	else
#endif
...
static int elan_ktf_ts_register_interrupt(struct i2c_client *client)
{
	...
	if(ekth3260_hw_id==2)
	{
		printk("[elan] %s:[%d]: request_threaded_irq WITHOUT IRQF_NO_SUSPEND.\n", __func__, __LINE__);
	...
	{
		printk("[elan] %s:[%d]: request_threaded_irq with IRQF_NO_SUSPEND.\n", __func__, __LINE__);
		err = request_threaded_irq(client->irq, NULL, elan_ktf_ts_irq_handler,
		                           IRQ_TYPE_EDGE_FALLING | IRQF_NO_SUSPEND | IRQF_ONESHOT,
		                           client->name, ts);
	}
	if(err)
	{
...
static int eKTH3260_parse_dt(struct device *dev, struct elan_ktf_ts_data *ts)
{
	...
	*/
	...
	printk("[elan] %s: ts->rst_gpio_gpio =%d, ts->intr_gpio =%d, ts->lcmid2_gpio=%d \n",
	       __func__, ts->rst_gpio, ts->intr_gpio, ts->lcmid2_gpio); // add by leo for testtest
	return 0;
}
...
// add by leo for cable status notifier ++
uint8_t AC_enable_cmd[4] = {0x54, 0x8f, 0x01, 0x01};
uint8_t PC_enable_cmd[4] = {0x54, 0x8f, 0x02, 0x01};
uint8_t AC_disable_cmd[4] = {0x54, 0x8f, 0x00, 0x01};
uint8_t AC_cmd[4] = {0};

static void elan_i2c_set(struct work_struct *work)
{
	int ret = 0;
	printk("ELAN %x %x %x %x\n", AC_cmd[0], AC_cmd[1], AC_cmd[2], AC_cmd[3]);
...
int eKTH3260_cable_status_handler(struct notifier_block *nb, unsigned long event, void *ptr)
{
	// add by leo for skip COS/POS/recovery/miniOS ++
	if(entry_mode != 1)
	{
		printk("%s:[%d]: Not in MOS, Skip cable status notifier\n", __func__, __LINE__);
		return NOTIFY_OK;
	}
	// add by leo for skip COS/POS/recovery/miniOS --
...
struct notifier_block eKTH3260_cable_status_notifier =
{
	.notifier_call = eKTH3260_cable_status_handler,
};
// add by leo for cable status notifier --
...
static ssize_t eKTH3260_proc_tp_debug_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
	...
	else if((buf[0] == 's') && (buf[1] == 't') && (buf[2] == 'r') && (buf[3] == 'e') && (buf[4] == 's') && (buf[5] == 's')) // stress
	{
			queue_work(ts->elan_irq_wq, &ts->irq_work);
	}
                             ...
                             const struct i2c_device_id *id)
{
	...
	printk("////////////////////		elan_ktf_ts_probe START v30 \n"); // add by leo for testtest
	...
	INIT_DELAYED_WORK(&ts->irq_delay_work, elan_ktf_ts_work_func);
	...
	*/
	...
	if(err < 0)
		printk(KERN_ERR "[elan] %s:Failed to request GPIO%d (ElanTouch-lcmid2) error=%d\n", __func__, ts->lcmid2_gpio, err);

	//ts->LCM_ID2 = gpio_get_value(ts->lcmid2_gpio);
	//printk("[elan] %s ts->lcmid2_gpio = %d \n", __func__, ts->LCM_ID2);

	ts->LCM_ID2 = panel_index;
	printk("[elan] %s panel_index = %d \n", __func__, ts->LCM_ID2);
	...
	*/
	...
	else
	{
		if(ts->LCM_ID2) // AUO LCM
		{
			...
			if((FW_ID != 0x3038) || (FW_VERSION < 0x5508))
			{
				printk("[elan] %s: Start touch FW update to version 0x3038_0x5508 for AUO LCM...\n", __func__);
		...
		else // CPT LCM
		{
			...
			if((FW_ID != 0x3037) || (FW_VERSION < 0x552E))
			{
				printk("[elan] %s: Start touch FW update to version 0x3037_0x552E for CPT LCM...\n", __func__);
				//elan_TWO_WIRE_ICE(ts->client);
				Update_FW_in_Driver(0);
...
// add by leo for early-suspend --
	...
	cable_status_register_client(&eKTH3260_cable_status_notifier); // add by leo for cable status notifier
...
static int elan_ktf_ts_remove(struct i2c_client *client)
{
	...
	cable_status_unregister_client(&eKTH3260_cable_status_notifier); // add by leo for cable status notifier
...
#if GESTURE_WAKEUP_BY_RESUME_FUNCTION
static int elan_ktf_gesture_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct elan_ktf_ts_data *ts = private_ts;
...
static int elan_ktf_gesture_resume(struct i2c_client *client)
{
	struct elan_ktf_ts_data *ts = private_ts;
...
#if GESTURE_WAKEUP_BY_RESUME_FUNCTION
	.suspend		= elan_ktf_gesture_suspend,
	.resume		= elan_ktf_gesture_resume,
#endif
...
static int elan_ktf_ts_init(void)
{
	...
	if(ekth3260_hw_id == 0)
	{
		//printk("////////////////////		probe ELAN eKTH3264 Touch driver on I2C4 for SR!!!\n");
		//return i2c_add_driver(&ektf_ts_driver_i2c4);
		printk("////////////////////		Skip I2C3 ELAN eKTH3264 Touch driver !!!\n");
		return 0;
	}
	else
	{
		printk("////////////////////		probe I2C3 ELAN eKTH3264 Touch driver !!!\n");
