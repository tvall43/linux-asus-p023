		...
		struct device_attribute *attr, char *buf)
{
	...
	if (data > MAX_FIFO_F_LEVEL) {
		LDBG("bma2x2 fifo_count abnormal, %d", data);

		if (bma2x2_get_mode(bma2x2->bma2x2_client, &mode) < 0)
			return -EINVAL;

		LDBG("bma2x2 fifo_count abnormal, op_mode: %d", mode);
...
/* Add by Tom for SR */
static const struct i2c_device_id bma2x2_id_SR[] = {
	{ "gsensor_sr", 0 },
	{ }
};

static const struct of_device_id bma2x2_of_match_SR[] = {
	{ .compatible = "bosch,bma2x2_SR", },
	{ },
};

static struct i2c_driver bma2x2_driver_SR = {
	.driver = {
		.owner  = THIS_MODULE,
		.name   = SENSOR_NAME,
		.of_match_table = bma2x2_of_match_SR,
	},
#ifndef CONFIG_PM_SCREEN_STATE_NOTIFIER
	.suspend    = bma2x2_suspend,
	.resume     = bma2x2_resume,
#endif
	.id_table   = bma2x2_id_SR,
	.probe      = bma2x2_probe,
	.remove     = bma2x2_remove,
	.shutdown   = bma2x2_shutdown,
};
...
static int __init BMA2X2_init(void)
{
	...
	if(hw_id == HW_ID_SR)
		return i2c_add_driver(&bma2x2_driver_SR);
	else 
