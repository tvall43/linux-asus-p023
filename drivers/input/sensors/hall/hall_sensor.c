...
extern int hall_status_notify(void);
...
static void lid_report_function(struct work_struct *dat)
{
	...
	msleep(DEBOUNCE_TIME);
        ...
        input_sync(hall_sensor_dev->lid_indev);
	wake_unlock(&Wake_Lock);
        if(!hall_sensor_dev->status) hall_trigger_suspend=true; 
		hall_status_notify();
        pr_info("[%s] SW_LID report value = %d\n", DRIVER_NAME,!hall_sensor_dev->status);

}
EXPORT_SYMBOL(hall_trigger_suspend);
...
static int set_irq_hall_sensor(void)
{
	...
	hall_sensor_dev->irq = IRQ;
	pr_info("[%s] hall_sensor irq = %d\n", DRIVER_NAME,hall_sensor_dev->irq);
	rc = request_irq(hall_sensor_dev->irq,hall_sensor_interrupt_handler,
			IRQF_SHARED|IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING|IRQF_NO_SUSPEND,"hall_sensor_irq",hall_sensor_dev);
	if (rc<0) {
...
static int lid_probe(struct platform_device *pdev){
        ...
        pr_info("[%d]B_irq_probe!\n",IRQ);
        if (IRQ == 0) {
                pr_info("irq_of_parse_and_map failed\n");
        }
...
{	
	int ret = 0;
        ...
        pr_info("++++++[%s]initial!++++++\n",DRIVER_NAME);
        pr_info("++++++Mode:[%d]initial!++++++\n",entry_mode);
	if(entry_mode == 4){
            pr_info("[%s]Disable hall sensor due to COS mode\n",DRIVER_NAME);
            return 0;
        }
        //insert pm_ops
...
MODULE_DESCRIPTION("Intel-SOFIA Hall sensor Driver");
