...
void bmm050_screen_changed_listener(const int state);
#endif
...
//static int bmm_power_ctl(struct bmm_client_data *data, bool on);
		...
		u8 *data, u8 len)
{
	...
	if (cadiz_support && is_enable_cadiz_power == 0) {
		LDBG("ByPass I2C Transfer , Becasuse Cadiz Power Not Ready\n");
		return -1;
	}
	while (0 != len--) {
#ifdef BMM_SMBUS
...
static ssize_t bmm_show_ecompass_status(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
	struct input_dev *input = to_input_dev(dev);
...
static DEVICE_ATTR(place, S_IRUGO,
		bmm_show_place, NULL);
static DEVICE_ATTR(ecompass_status, S_IRUGO,
		bmm_show_ecompass_status, NULL);
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("===== E-commpass Probe Start V1.0.2 =====\n");

	if (cadiz_support) {
		cadiz_power_control(ECOMPASS, true);
...
*/
	...
	/* Add by Tom for create sysfs in sys/bus/i2c/devices/0-0012 */
	...
	// reg callback
#ifdef CONFIG_PM_SCREEN_STATE_NOTIFIER
	callback_struct = register_screen_state_notifier(&bmm050_screen_changed_listener);
#endif
	PNOTICE("sensor %s probed successfully", SENSOR_NAME);


	printk("===== E-commpass Probe End =====\n");
	return 0;
...
static void bmm_early_suspend(struct early_suspend *handler)
{
	...
	if (cadiz_support) {
		...
		cadiz_power_control(ECOMPASS, false);
	}
...
/* Added by alp for ER */
static const struct i2c_device_id bmm_id_ER[] = {
	{"ecompass_er", 0},
	{}
};

static const struct of_device_id bmm050_of_match_ER[] = {
	{ .compatible = "bosch,bmm050_ER", },
	{ },
};

static struct i2c_driver bmm_driver_ER = {
	.driver = {
		.owner = THIS_MODULE,
		.name = SENSOR_NAME,
		.of_match_table = bmm050_of_match_ER,
	},
...
static int __init BMM_init(void)
{
	...
	if(entry_mode != 1)	{	// entry_mode==1  ==> MOS mode
		printk(KERN_ERR "alp.D : Non MOS mode , Disable BMM Senosr , mode=%d\n",entry_mode);
		return 0;
	}
	...
	if(pj_id == 0x1){
		...
		if(hw_id == HW_ID_ER)
			return i2c_add_driver(&bmm_driver_ER);
		else 
			return i2c_add_driver(&bmm_driver);
	}
}
