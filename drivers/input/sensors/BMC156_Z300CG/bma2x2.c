...
#ifdef CONFIG_FLICK
#define DEFAULT_FLICK_JUDGE_PERIOD 1000    	/* default judge in 1 second */
...
#define CONFIG_ASUS_ATD 1
/* Add by Tom for Calibration */
#define GS_INI_PATH "/nvm_fs_partition/sensors/gs_cali.ini"
						...
						unsigned char enabled_mode)
{
	...
	if (mode < 6) {
		...
		case BMA2X2_MODE_SUSPEND:
			...
			if (ret < 0)
				LDBG(KERN_ERR "Error switching to suspend");
				break;
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	LDBG("Set Sensor status [%d] , pre_enable [%d]\n", enable, pre_enable);
	
	// Add by for Cadiz Power issue
    	mutex_lock(&bma2x2_cadiz_lock);
	if (cadiz_support && is_enable_cadiz_power == 0) {
		cadiz_power_control(GSENSOR, true);
	...
	// Add by for Cadiz Power issue
	if (cadiz_support && is_enable_cadiz_power == 1 && is_suspend == 1) {
		is_enable_cadiz_power = 0;
		is_suspend = 0;
		cadiz_power_control(GSENSOR, false);
		LDBG("Disable Cadiz Power\n");
	}
    	mutex_unlock(&bma2x2_cadiz_lock);
		...
		const char *buf, size_t count)
{
	...
	error = kstrtoul(buf, 10, &data);
	LDBG("Set Sensor Status [%ld] from ATTR\n", data);
	if (error)
		return error;
	if ((data == 0) || (data == 1))
...
static ssize_t bma2x2_enable_flick_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (data == 1 && atomic_read(&bma2x2->en_flick_hv) == 0) {
		...
		bandwidth = BMA2X2_SET_BITSLICE(bandwidth, BMA2X2_BANDWIDTH, BMA2X2_BW_125HZ);
		err += bma2x2_smbus_write_byte(bma2x2->bma2x2_client, BMA2X2_BANDWIDTH__REG, &bandwidth);
		...
		struct device_attribute *attr, char *buf)
{
	...
	LDBG("result [%d] , ret [%d]", result, ret);
	if((ret == 0) && (result == 0))	
...
static ssize_t bma2x2_enable_terminal_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (data == 1) {
		...
		if(err == 0) {
			atomic_set(&bma2x2->terminal, 1);
			LDBG("Enable Terminal function from AP\n");
		} else {
			atomic_set(&bma2x2->terminal, 0);
...
// Add by Tom for Test Rotation
static ssize_t bma2x2_rotation_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data = 0;
...
static ssize_t bma2x2_rotation_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	switch (data) {
		case 0:
			LDBG("Test : Rotation to Positive Portait \n");
			x = 0;
			y = 512;
			z = 0;
			break;
		case 1:
			LDBG("Test : Rotation to Positive Lanscape \n");
			x = 512;
			y = 0;
			z = 0;
			break;
		case 2:
			LDBG("Test : Rotation to Negative Portait \n");
			x = 0;
			y = -512;
			z = 0;
			break;
		case 3:
			LDBG("Test : Rotation to Negative Lanscape \n");
			x = -512;
			y = 0;
			z = 0;
			break;
	...
	for(i = 0 ; i < 50 ; i++)
	{
		input_report_abs(bma2x2->input, ABS_X, x);
		input_report_abs(bma2x2->input, ABS_Y, y);
		input_report_abs(bma2x2->input, ABS_Z, z);
		input_sync(bma2x2->input);
		mdelay(50);
	}
	return count;
...
// Add by Tom for Test Rotation 
static DEVICE_ATTR(rotation, S_IRUGO|S_IWUSR|S_IWGRP|S_IROTH,
		bma2x2_rotation_show, bma2x2_rotation_store);
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	case 0x02:
		...
		if (bma2x2->flick_debounce_time == 0) {
			...
			LDBG("Flick debounce , Bypass Event !\n");
			break;
		}
		// LDBG("flick Status [%d] , flick_times[%d] \n", atomic_read(&bma2x2->en_flick), bma2x2->flick_times);
		...
		if (atomic_read(&bma2x2->en_flick) == 1 || atomic_read(&bma2x2->en_flick_hv) == 1) {
			...
			*/
			...
			if (bma2x2->flick_times == 0)	{
				LDBG("[Single] flick interrupt happened \n");
				...
				/* Report event */
				LDBG("[Double] flick interrupt happened \n");
	...
	case 0x40:
		bma2x2_get_orient_status(bma2x2->bma2x2_client,
				    &first_value);
		LDBG("orient interrupt happened, [%s]\n",
				orient[first_value]);
		if (first_value == 0){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, DOWNWARD_LANDSCAPE_LEFT_INTERRUPT_HAPPENED);
		}
		else if (first_value == 1){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, DOWNWARD_LANDSCAPE_RIGHT_INTERRUPT_HAPPENED);
		}
		else if (first_value == 2){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, DOWNWARD_PORTRAIT_UP_INTERRUPT_HAPPENED);
		}
		else if (first_value == 3){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, DOWNWARD_PORTRAIT_DOWN_INTERRUPT_HAPPENED);
		}
		else if (first_value == 4){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, UPWARD_LANDSCAPE_LEFT_INTERRUPT_HAPPENED);
		}
		else if (first_value == 5){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, UPWARD_LANDSCAPE_RIGHT_INTERRUPT_HAPPENED);
		}
		else if (first_value == 6){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, UPWARD_PORTRAIT_UP_INTERRUPT_HAPPENED);
		}
		else if (first_value == 7){
			input_report_abs(bma2x2->dev_terminal, ORIENT_INTERRUPT, UPWARD_PORTRAIT_DOWN_INTERRUPT_HAPPENED);
		}
		input_sync(bma2x2->dev_terminal);
		break;
	case 0x80:
		bma2x2_get_orient_flat_status(bma2x2->bma2x2_client,
				    &sign_value);
		LDBG("flat interrupt happened,flat status is %d\n",
				    sign_value);
		if (sign_value == 1) {
		...
		const struct i2c_device_id *id)
{
	...
	// Workaround For SR / ER i2c Change
	client->addr = 0x11;

	printk("===== G-Sensor Probe Start V1.0.1 =====\n");
	...
	if (bma2x2_soft_reset(client) < 0) {
		LDBG("bma2x2_soft_reset fail\n");
		for( i = 0 ; i < 10 ; i++) {
			mdelay(20);
			if(bma2x2_soft_reset(client) >= 0) {
				LDBG("i2c retry pass , retry times %d\n",i);
				break;
			} else { 
				LDBG("i2c bus write error, retry times %d\n",i);
			}
		}
		if(i == 10) {
			LDBG("i2c bus write error, pls check HW connection\n");
			err = -EINVAL;
			goto kfree_exit;
		}
	}
	RESET_DELAY();
	LDBG("bma2x2_soft_reset pass\n");
...
#if defined(BMA2X2_ENABLE_INT1) || defined(BMA2X2_ENABLE_INT2)
	printk(KERN_INFO "alp.D : check config INT\n");
	...
	data->int_gpio= of_get_named_gpio(client->dev.of_node, "bosch,gsensor-gpio", 0);	
//	if(data->int_gpio>0){
//		printk(KERN_INFO "alp.D : init interrupt pin(%d)\n",data->int_gpio);
		// init interrupt pin	
		err = gpio_request(data->int_gpio, "gsensor-bma2x2-gpio");
		if(err < 0)
			printk(KERN_ERR "alp.D : bosch Failed to request GPIO%d (gsensor-bma2x2-gpio) error=%d\n", data->int_gpio, err);
//		data->IRQ = irq_of_parse_and_map(client->dev.of_node,0);
//	}
//	data->IRQ = client->irq;
	data->IRQ = irq_of_parse_and_map(client->dev.of_node,0);
	printk(KERN_INFO "alp.D : if - request_irq=%d\n",data->IRQ);
	err = request_irq(data->IRQ, bma2x2_irq_handler, IRQF_TRIGGER_RISING, "bma2x2", data);

	#ifdef CONFIG_SIG_MOTION
	printk(KERN_INFO "alp.D : enable_irq_wake\n");
	...
	setup_timer(&data->flick_timer, bma2x2_flick_timeout_handle,
			(unsigned long)data);
	bma2x2_set_high_g_threshold(g_accel_data->bma2x2_client, 80);
#endif
...
static int bma2x2_suspend(struct i2c_client *client)
{
	...
	if (cadiz_support) {
    	...
    	mutex_unlock(&bma2x2_cadiz_lock);
	LDBG("Suspend , Enable Status [%d] is_enable_cadiz_power [%d]\n", atomic_read(&data->enable), is_enable_cadiz_power);
	return 0;
}
...
static int bma2x2_resume(struct i2c_client *client)
{
	...
	if (atomic_read(&data->enable) == 1) {
		bma2x2_set_mode(data->bma2x2_client,
			BMA2X2_MODE_NORMAL, BMA_ENABLED_INPUT);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
		LDBG("Start Polling, Delay [%ld]\n", msecs_to_jiffies(atomic_read(&data->delay)));
		schedule_delayed_work(&data->work,
				msecs_to_jiffies(atomic_read(&data->delay)));
#endif
	}
	mutex_unlock(&data->enable_mutex);
	LDBG("Resume , Enable Status [%d] is_enable_cadiz_power [%d]\n", atomic_read(&data->enable), is_enable_cadiz_power);
...
void bma2x2_screen_chenged_listaner(const int state)
{
	if(state == NOTIFY_WHEN_SCREEN_OFF)
	{
		...
		bma2x2_suspend(g_accel_data->bma2x2_client);
	}
	else if(state == NOTIFY_WHEN_SCREEN_ON)
	{
		/* something you want to do at screen on*/
		bma2x2_resume(g_accel_data->bma2x2_client);
	}
}
#endif
...
/* Added by alp for ER */
static const struct i2c_device_id bma2x2_id_ER[] = {
	{ "gsensor_er", 0 },
	{ }
};

static const struct of_device_id bma2x2_of_match_ER[] = {
	{ .compatible = "bosch,bma2x2_ER", },
	{ },
};

static struct i2c_driver bma2x2_driver_ER = {
	.driver = {
		.owner  = THIS_MODULE,
		.name   = SENSOR_NAME,
		.of_match_table = bma2x2_of_match_ER,
	},
	.suspend    = bma2x2_suspend,
#ifndef CONFIG_PM_SCREEN_STATE_NOTIFIER
	.resume     = bma2x2_resume,
#endif
	.id_table   = bma2x2_id_ER,
	.probe      = bma2x2_probe,
	.remove     = bma2x2_remove,
	.shutdown   = bma2x2_shutdown,
};
...
static int __init BMA2X2_init(void)
{
	...
	int pj_id = Read_PROJ_ID();

	// added by cheng_kao to skip COS/POS
	if(entry_mode != 1)	{	// entry_mode==1  ==> MOS mode
		printk(KERN_ERR "alp.D : Non MOS mode , Disable BMA Senosr , mode=%d\n",entry_mode);
		return 0;
	}
	// added by alp for CNG = 0x1
	if(pj_id == 0x1){
		return i2c_add_driver(&bma2x2_driver);
	}else {	// Z300CG=0x2, Z300C=0x6, Z381CG=0x0, Z381C=0x4 ; Z300CNG=0x1
		if(hw_id == HW_ID_ER)
			return i2c_add_driver(&bma2x2_driver_ER);
		else 
			return i2c_add_driver(&bma2x2_driver);
	}
}
