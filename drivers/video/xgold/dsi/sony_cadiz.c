...
#define CADIZ_I2C_REG 1

static struct i2c_client *cadiz_client;
static bool skip_first_workaround = true;
static bool cadiz_ready = false;

#if defined(CONFIG_Z300C) || defined(CONFIG_Z300CG)
int cadiz_1v2_ldo_en;  //LVDS_TCLK1N    gpio74
int cadiz_1v8_ldo_en;  //LVDS_TD1P      gpio83
int cadiz_osc_en;      //DIF_D21        gpio71
#elif defined(CONFIG_Z380C) || defined(CONFIG_Z380CG)
int cadiz_pwr_en;      //DIF_D4         gpio48
#else
int cadiz_ldo_enable;
#endif

int cadiz_rst;     //Z300C/CG: LVDS_TB1N(gpio78) //Z380C: DIF_D2(gpio46)
int cadiz_id_pin;  //Z300C/CG: NAND_CS0(gpio34)  //Z380C: DIF_D21(gpio71)

static struct miscdevice cadiz_dev;

static struct dentry *cadiz_debugfs;

bool cadiz_support = true;
EXPORT_SYMBOL(cadiz_support);
...
initcode cadiz_boot1[]= {
	...
	{0x025A,0x00},
	{0x025B,0x00},
	{0x025C,0x07},	//BIT1 clock mode: 1=non-continuous, 0=continuous
	{0x025D,0x00},
	{0x025E,0x00},
	{0x025F,0x00},
	{0x0260,0x0C},
...
int cadiz_get_auo_boot1(void)
{
	...
	for (i = 0; i < arraysize; i++) {
		cadiz_boot1[i].addr = z300_auo_boot1[i].addr;
		cadiz_boot1[i].param = z300_auo_boot1[i].param;
	}
...
int cadiz_dts_parser(struct i2c_client *client){

#if defined(CONFIG_Z300C) || defined(CONFIG_Z300CG)
	cadiz_1v2_ldo_en = of_get_named_gpio_flags(client->dev.of_node, "cadiz,1v2-ldo-en",0, NULL);
	if (gpio_request(cadiz_1v2_ldo_en, "cadiz_1v2_ldo_en") != 0)
		printk("[DISPLAY] %s: request cadiz_1v2_ldo_en failed \n", __func__);

	cadiz_1v8_ldo_en = of_get_named_gpio_flags(client->dev.of_node, "cadiz,1v8-ldo-en",0, NULL);
	if (gpio_request(cadiz_1v8_ldo_en, "cadiz_1v8_ldo_en") != 0)
		printk("[DISPLAY] %s: request cadiz_1v8_ldo_en failed \n", __func__);

	//SR don't care CDZ_OSC_EN pin
	if (hw_id != HW_ID_SR) {
		cadiz_osc_en = of_get_named_gpio_flags(client->dev.of_node, "cadiz,osc-en",0, NULL);
		if (gpio_request(cadiz_osc_en, "cadiz_osc_en") != 0)
			printk("[DISPLAY] %s: request cadiz_osc_en failed \n", __func__);
	}

#elif defined(CONFIG_Z380C) || defined(CONFIG_Z380CG)
	cadiz_pwr_en = of_get_named_gpio_flags(client->dev.of_node, "cadiz,pwr-en",0, NULL);
	if (gpio_request(cadiz_pwr_en, "cadiz_pwr_en") != 0)
		printk("[DISPLAY] %s: request cadiz_pwr_en failed \n", __func__);

#else
	cadiz_ldo_enable = of_get_named_gpio_flags(client->dev.of_node, "cadiz,lcm-en-gpio",0, NULL);
	if (gpio_request(cadiz_ldo_enable, "cadiz_ldo_enable") != 0)
		printk("[DISPLAY] %s: request cadiz_ldo_enable failed \n", __func__);
#endif

	cadiz_rst = of_get_named_gpio_flags(client->dev.of_node, "cadiz,rst-gpio",0, NULL);
...
//workaround for i2c bus
int cadiz_workaround(void){

	//printk("[DISPLAY] %s: Enter\n",__func__);

	if(!cadiz_support) return 0;

#if defined(CONFIG_Z300C) || defined(CONFIG_Z300CG)
	if (cadiz_osc_en) {
		gpio_direction_output(cadiz_osc_en, 1);
	}
	...
	if (cadiz_osc_en) {
		gpio_direction_output(cadiz_osc_en, 0);
		usleep_range(1000, 1000);
	}
	if (cadiz_rst) {
...
int save_power_status = 0x0;
struct mutex cadiz_power_mutex;
int cadiz_power_control(int owner, bool enable){
	int power_status;

	mutex_lock(&cadiz_power_mutex);

	if(enable){
		power_status = save_power_status | owner;
	}else{
		power_status = save_power_status & ~ (owner);
	}

	printk("[DISPLAY] %s: owner = 0x%02x, enable = %d (0x%02x => 0x%02x)\n",
		__func__, owner, enable, save_power_status, power_status);

	if(!!save_power_status == !! power_status){
		goto exit;
	}
...
#if defined(CONFIG_Z300C) || defined(CONFIG_Z300CG)
		if (cadiz_1v2_ldo_en) {
			gpio_direction_output(cadiz_1v2_ldo_en, 1);
			usleep_range(500, 500);
		}
		if (cadiz_1v8_ldo_en) {
			gpio_direction_output(cadiz_1v8_ldo_en, 1);
		}
		if (support_loader_display() && skip_first_workaround) {
			power_status = power_status | CADIZ;
			skip_first_workaround = false;
			cadiz_ready = true;
		}
		else
			cadiz_workaround();
...
#elif defined(CONFIG_Z380C) || defined(CONFIG_Z380CG)
		if (cadiz_pwr_en) {
			gpio_direction_output(cadiz_pwr_en, 1);
		}
...
#if defined(CONFIG_Z300C) || defined(CONFIG_Z300CG)
		if (cadiz_1v8_ldo_en) {
			gpio_direction_output(cadiz_1v8_ldo_en, 0);
			usleep_range(500, 500);
		}
		if (cadiz_1v2_ldo_en) {
			gpio_direction_output(cadiz_1v2_ldo_en, 0);
		}
#elif defined(CONFIG_Z380C) || defined(CONFIG_Z380CG)
		if (cadiz_pwr_en) {
			gpio_direction_output(cadiz_pwr_en, 0);
		}
#else
		if (cadiz_ldo_enable)
...
exit:
	...
	mutex_unlock(&cadiz_power_mutex);
	return 0;
}
...
void cadiz_resume(void)
{
	...
	cadiz_power_control(CADIZ, true);
...
static int cadiz_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (IS_ERR(cadiz_debugfs)){
		...
		if (hw_id == HW_ID_SR || hw_id == HW_ID_ER) {
			debugfs_create_symlink("read_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz-sr/3-0010/cadiz/read_cadiz_i2c");
			debugfs_create_symlink("write_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz-sr/3-0010/cadiz/send_cadiz_i2c");
		} else {
			debugfs_create_symlink("read_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz/4-0010/cadiz/read_cadiz_i2c");
			debugfs_create_symlink("write_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz/4-0010/cadiz/send_cadiz_i2c");
		}
#elif defined(CONFIG_Z380C)
		if (hw_id == HW_ID_SR) {
			debugfs_create_symlink("read_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz-sr/3-0011/cadiz/read_cadiz_i2c");
			debugfs_create_symlink("write_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz-sr/3-0011/cadiz/send_cadiz_i2c");
		} else {
			debugfs_create_symlink("read_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz/4-0011/cadiz/read_cadiz_i2c");
			debugfs_create_symlink("write_reg",cadiz_debugfs,
					"/sys/bus/i2c/drivers/cadiz/4-0011/cadiz/send_cadiz_i2c");
		}
#endif
	...
	//mutex init
	mutex_init(&cadiz_power_mutex);

	//dts parser
	cadiz_dts_parser(client);
...
static int cadiz_sr_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	printk("[DISPLAY] %s: Enter\n",__func__);
	cadiz_probe(client, id);

	return 0;
}

static struct of_device_id cadiz_sr_i2c_table[] = {
	{.compatible = "sony,cadiz-sr"}, //Compatible node must match dts
	{},
};
static const struct i2c_device_id cadiz_sr_id[] = {
	{ "cadiz-sr", 0 },
	{ },
};
static struct i2c_driver cadiz_sr_driver = {
	.driver = {
		.name = "cadiz-sr",
		.owner = THIS_MODULE,
		.of_match_table = cadiz_sr_i2c_table,
	},
	.probe = cadiz_sr_probe,
	.id_table = cadiz_sr_id,
};

int cadiz_id_dts_parser(void)
{
	struct device_node *cadiz_np;
	int ret;

	cadiz_np = of_find_matching_node(NULL, cadiz_i2c_table);
	if (!cadiz_np) {
		pr_err("%s: Can't find CADIZ i2c node\n", __func__);
		return -EINVAL;
	}

	cadiz_id_pin = of_get_named_gpio_flags(cadiz_np, "cadiz,id", 0, NULL);
	if (gpio_is_valid(cadiz_id_pin)) {
		ret = gpio_request(cadiz_id_pin, "cadiz_id");
		if (ret) {
			pr_err("%s: request CADIZ ID gpio fail: %d\n",
			       __func__, ret);
			cadiz_id_pin = 0;
		}
	} else {
...
int get_cadiz_id(void)
{
	int ret = 0;

	if (!cadiz_id_pin) {
		pr_err("%s: CADIZ ID pin fail\n", __func__);
		return -EINVAL;
	}

	ret = gpio_get_value(cadiz_id_pin);
	printk("[DISPLAY] %s: %d\n",__func__, ret);
...
static int __init cadiz_I2C_init(void)
{
	...
	if (hw_id != HW_ID_SR) {
		cadiz_id_dts_parser();
		cadiz_id = get_cadiz_id();
	}

	if (proj_id == PROJ_ID_Z380C) {
		//for Z380C
		if (hw_id == HW_ID_SR) {
			ret = i2c_add_driver(&cadiz_sr_driver);
		} else {
			if (cadiz_id == 1)
				ret = i2c_add_driver(&cadiz_driver);
			else {
				cadiz_support = false;
				printk("[DISPLAY] %s: NO CADIZ sku\n",__func__);
				return 0;
			}
		}
	} else {
