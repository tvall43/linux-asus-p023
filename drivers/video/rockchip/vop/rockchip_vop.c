...
struct rockchip_vop_driver *rockchip_dev;
EXPORT_SYMBOL(rockchip_dev);

bool vop_shutdown = false;
EXPORT_SYMBOL(vop_shutdown);
...
#ifdef CONFIG_TI_LP8557I
	dev_info(dev_drv->dev, "[DISPLAY] lp8557i_resume (entry_mode = %d)\n", entry_mode);
	lp8557i_resume();
#endif
...
static void rockchip_vop_shutdown(struct platform_device *pdev)
{
	...
	if (cadiz_support) {
		...
		// force turn off cadiz 1v2, 1v8 power
		cadiz_power_control(0xe, false);
#endif
	}
#endif
