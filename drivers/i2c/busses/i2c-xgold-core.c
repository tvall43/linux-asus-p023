 ...
 * FUNCTION PROTOTYPE
 *****************************************************************************/
static int xgold_i2c_core_reset(struct i2c_adapter *adap);
...
static irqreturn_t xgold_i2c_irq_handler(int irq, void *dev)
{
	...
	do {
		i2c_debug(data, "%s: mis = 0x%X, last_mis:0x%X, loop: %d\n",
				__func__, mis, last_mis, loop++);
		...
		if (mis == last_mis) {
			if (cnt++ > 32) {
				dev_err(&adap->dev, "%s: reset in irq! loop: %d\n",
						__func__, loop);
				dev_err(&adap->dev, "mis: 0x%x, slave: 0x%x\n",
						mis, data->addr);
				spin_lock(&data->lock);
				xgold_i2c_core_reset(adap);
				spin_unlock(&data->lock);
			}
		} else {
			cnt = 0;
...
static int xgold_i2c_xfer_msg(struct i2c_adapter *adap, struct i2c_msg *msg)
{
	...
	if (I2C_MSG_RD(data->flags)) {
		...
		if (data->dma_mode == true && (ret == 0 || data->cmd_err) &&
			...
			dev_err(&adap->dev, "timeout waiting end of cmd RD, slave: 0x%x\n",
					data->addr);
			data->cmd_err = -ETIMEDOUT;
		}
		...
		if (data->dma_mode == true && (ret == 0 || data->cmd_err) &&
			...
			dev_err(&adap->dev, "timeout waiting end of cmd WR, slave: 0x%x\n",
					data->addr);
			data->cmd_err = -ETIMEDOUT;
		}
...
out:
	...
	if (data->cmd_err) {
		...
		reg_write(I2C_ICR_LSREQ_INT_CLR_INT | I2C_ICR_SREQ_INT_CLR_INT |
				...
				I2C_ICR_BREQ_INT_CLR_INT,
				data->regs + I2C_ICR_OFFSET);
		dev_err(&adap->dev, "%s: reset in cmd error!\n", __func__);
		/* do i2c reset here to recover */
		spin_lock_irqsave(&data->lock, flag);
		xgold_i2c_core_reset(adap);
		spin_unlock_irqrestore(&data->lock, flag);
...
static int xgold_i2c_core_reset(struct i2c_adapter *adap)
{
	...
	struct xgold_i2c_platdata *platdata = dev_get_platdata(i2c_dev->dev);

	dev_err(&adap->dev, "%s: Xgold I2C reset, slave: 0x%x\n",
			__func__, data->addr);
	xgold_i2c_hw_stop(data);
	/* delay 50ms for trial */
	mdelay(50);
	if (platdata->rst)
		reset_control_reset(platdata->rst);
