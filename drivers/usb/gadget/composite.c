...
//2015 Nick :Add for factory test 1022
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_GET_DESCRIPTOR:
		...
		case USB_DT_DEVICE:
			if (Read_PROJ_ID()==PROJ_ID_Z300C || Read_PROJ_ID()==PROJ_ID_Z300CG || Read_PROJ_ID()==PROJ_ID_Z380C){
				if(factory_mode == 2){
				...
				if(!strcmp(original_usb_serial_number,"unknown")){
					string_override(composite->strings,
                                        cdev->desc.iSerialNumber, desc_serial_number);
				}else{
					string_override(composite->strings,
