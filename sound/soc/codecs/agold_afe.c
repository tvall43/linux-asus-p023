...
#define PROP_SPK_AMP_PIN	75
#else
#define PROP_SPK_AMP_PIN        78
#endif
...
static ssize_t sp_show(struct device *dev, struct device_attribute *attr,
                        char *buf);
static ssize_t sp_store(struct device *dev, struct device_attribute *attr,
                        const char *buf, size_t count);
static DEVICE_ATTR(sp_amp_status, S_IWUSR | S_IRUGO, sp_show, sp_store);

static ssize_t sp_show(struct device *dev, struct device_attribute *attr,
                char *buf)
{
        //printk("joe hs_show \n");
        return sprintf(buf, "%d\n", gpio_get_value_cansleep(PROP_SPK_AMP_PIN));
}
                ...
                const char *buf, size_t count)
{
        ...
        if (buf[0] == '0') {
                gpio_set_value_cansleep(PROP_SPK_AMP_PIN, 0);
                printk("%s SP_AMP_EN_GPIO%d 0\n", __func__,PROP_SPK_AMP_PIN);
        }
        else if (buf[0] == '1') {
                gpio_set_value_cansleep(PROP_SPK_AMP_PIN, 1);
                printk("%s SP_AMP_EN_GPIO%d 1\n", __func__,PROP_SPK_AMP_PIN);
        }
        return 1;
}
			...
			struct snd_ctl_elem_value *uinfo)
{
	...
	afe_debug("%s: num_gains =  %d\n", __func__, hs_ofs_data.num_gains);
	/* If num_gains eq 0xffff use default analog gain table to calibrate */
	if (0xffff != hs_ofs_data.num_gains) {
		hs_ofs_data.length = hs_ofs_control->length;
...
static int32_t agold_afe_set_spk_gpio(struct device *dev, bool on)
{
	struct agold_afe_data *agold_afe = dev_get_drvdata(dev);

        //afe_err("Eddy, %s:\n", __func__);

	if (agold_afe->spk_gpio > 0) {
		gpio_direction_output(agold_afe->spk_gpio, on);
		//afe_err("Eddy, %s: %d\n", __func__, on);
	}
	return 0;
}
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		afe_debug("%s: SND_SOC_DAPM_PRE_PMU\n", __func__);
                if(PROJ_ID == 0x4){
                	if(HW_ID == 0){
                   		//afe_err("Eddy Z380C sr");
                   		agold_afe->set_spk_gpio(w->codec->dev, false);
                	}else{
                   		//afe_err("Eddy Z380C er");
                   		agold_afe->set_spk_gpio(w->codec->dev, true);
                	}
		}else{
                	//afe_err("Eddy Z300C Z300CG");
			agold_afe->set_spk_gpio(w->codec->dev, true);
                }
		break;
...
static int agold_afe_codec_probe(struct snd_soc_codec *codec)
{
	...
	afe_debug("%s\n", __func__);
        ...
        afe_err("Eddy PROJ_ID %d\n",PROJ_ID);

        HW_ID = Read_HW_ID();
        afe_err("Eddy HW_ID %d\n",HW_ID);
...
skip_pinctrl:
	...
	agold_afe_set_pinctrl_state(&pdev->device, agold_afe->pins_inactive);
        ...
        //agold_afe->set_spk_gpio = agold_afe_set_spk_gpio;
	agold_afe->spk_gpio = PROP_SPK_AMP_PIN;
	ret = gpio_request(agold_afe->spk_gpio, PROP_SPK_AMP_NAME);
	if (!ret) {
		afe_debug("req gpio_request success!:%d\n", ret);
	} else {
		afe_err("req gpio_request failed:%d\n", ret);
	}
        agold_afe->set_spk_gpio = agold_afe_set_spk_gpio;

        ret = device_create_file(&pdev->device, &dev_attr_sp_amp_status);
        if (ret < 0)
                afe_err("%s(): add sp_amp_status error\n", __func__);
