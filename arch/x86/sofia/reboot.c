...
static void next_mode_transition(enum boot_mode *next_mode, char *reason)
{
	...
	if (!reason)
		...
		*next_mode = ENUM_BOOT_MODE_RECOVERY_CLEAR;
        else if (strcmp(reason, "fastboot") == 0 || strcmp(reason, "bootloader") == 0) // Jui mod
		...
		*next_mode = ENUM_BOOT_MODE_PTEST_CLEAR;
        else if (strcmp(reason, "charger") == 0)
                *next_mode = ENUM_BOOT_MODE_CHARGEONLY;
	else
		*next_mode = ENUM_BOOT_MODE_NORMAL;
...
{
	volatile enum boot_mode next_mode = 0;
	...
	while (1) {
		...
		if (running_guest != temp)
			printk("[test]running_guest=%d share_data->os_id=%d\n",running_guest,share_data->os_id);
		temp = running_guest;
