...
const char* Read_HW_ID_STR(void)
{
        switch (hardware_id) {
        case HW_ID_SR:
                return HW_ID_SR_STR;
        ...
        return HW_ID_UNKNOWN_STR;
}
EXPORT_SYMBOL(Read_HW_ID_STR);
...
//Mini-porting ++
        entry_mode = 1;
        loc_main = strstr(boot_command_line,"androidboot.mode=normal");
        if (loc_main != NULL) {
                entry_mode = 1;
                printk(KERN_NOTICE "string match androidboot.mode=normal.\n");
        }
        loc_fato = strstr(boot_command_line,"androidboot.mode=recovery");
        if (loc_fato != NULL) {
                entry_mode = 2;
                printk(KERN_NOTICE "string match androidboot.mode=recovery.\n");
        }
        ...
        loc_minios = strstr(boot_command_line,"androidboot.mode=ptest");
        if (loc_minios != NULL) {
                entry_mode = 5;
                printk(KERN_NOTICE "string match androidboot.mode=ptest\n");
        }
